...
Since 5.6.*:
```
check General setup -> Configure standard kernel features (expert users). This will toogle the "Fully preemptible kernel).
```
...
On a PREEMPT_RT kernel, max latency > 1 ms under load indicates a configuration problem (SMIs, wrong IRQ affinity, C-states, etc.).
...
SMIs are non-maskable hardware interrupts that bypass the RT kernel entirely. They are common on laptops and can cause spikes of several ms.
```
$ sudo modprobe msr
...
Mitigations: disable unused hardware in BIOS (Bluetooth, fingerprint reader, unused USB controllers), apply firmware updates.
