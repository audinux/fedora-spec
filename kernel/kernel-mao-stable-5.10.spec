...
cp %{SOURCE1} .config
sed -i -e "s/EXTRAVERSION =/EXTRAVERSION = -rt-stable%{krt}.%{krel}%{fcver}/g" Makefile
...
dd if=/dev/zero of=%{buildroot}/boot/initramfs-%{kver}-rt-stable%{krt}.%{krel}%{fcver}.img bs=1M count=20

cp $KBUILD_IMAGE %{buildroot}/boot/vmlinuz-%{kver}-rt-stable%{krt}.%{krel}%{fcver}
chmod a+x %{buildroot}/boot/vmlinuz-%{kver}-rt-stable%{krt}.%{krel}%{fcver}

make %{?_smp_mflags} INSTALL_HDR_PATH=%{buildroot}/usr KBUILD_SRC= headers_install
cp System.map %{buildroot}/boot/System.map-%{kver}-rt-stable%{krt}.%{krel}%{fcver}
cp .config    %{buildroot}/boot/config-%{kver}-rt-stable%{krt}.%{krel}%{fcver}

cp %{buildroot}/boot/vmlinuz-%{kver}-rt-stable%{krt}.%{krel}%{fcver} %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/vmlinuz

rm -f %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build
rm -f %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/source

mkdir -p %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build
(cd %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver} ; ln -s build source)

# dirs for additional modules per module-init-tools, kbuild/modules.txt
mkdir -p %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/extra
mkdir -p %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/internal
mkdir -p %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/updates
...
cp --parents $(find -type f \( -name "Makefile*" -o -name "Kconfig*" \)) %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build
cp Module.symvers %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build
cp System.map %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build
if [ -s Module.markers ]; then
  cp Module.markers %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build
fi

# Move the devel headers out of the root file system

DevelDir=/usr/src/kernels/%{kver}-rt-stable%{krt}.%{krel}%{fcver}

mkdir -p %{buildroot}/usr/src/kernels/%{kver}-rt-stable%{krt}.%{krel}%{fcver}
mv %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build %{buildroot}/$DevelDir
...
ln -sf $DevelDir %{buildroot}/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/build
...
tar $EXCLUDES -cf- . | (cd %{buildroot}/usr/src/kernels/%{kver}-rt-stable%{krt}.%{krel}%{fcver}; tar xvf -)

%post
# Create the initramfs file; kernel-install also updates the bootloader (BLS)
/bin/kernel-install add %{kver}-rt-stable%{krt}.%{krel}%{fcver} /lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/vmlinuz
...
if [ "$1" = "0" ]; then
    /bin/kernel-install remove %{kver}-rt-stable%{krt}.%{krel}%{fcver} /lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}/vmlinuz
fi

%files
/lib/modules/%{kver}-rt-stable%{krt}.%{krel}%{fcver}
/boot/vmlinuz-%{kver}-rt-stable%{krt}.%{krel}%{fcver}
/boot/System.map-%{kver}-rt-stable%{krt}.%{krel}%{fcver}
/boot/config-%{kver}-rt-stable%{krt}.%{krel}%{fcver}
%ghost /boot/initramfs-%{kver}-rt-stable%{krt}.%{krel}%{fcver}.img

%files headers
/usr/include

%files devel
/usr/src/kernels/%{kver}-rt-stable%{krt}.%{krel}%{fcver}

%changelog
* Wed Aug 26 2026 Yann Collette <ycollette.nospam@free.fr> - 5.10.235-rt129-16
- fix invalid version string: use '.' instead of '-' before %%{krel}

* Wed Aug 26 2026 Yann Collette <ycollette.nospam@free.fr> - 5.10.235-rt129-15
...
* Thu Jan 23 2025 Yann Collette <ycollette.nospam@free.fr> - 5.10.233-rt125-12
- update to 5.10.233-rt125-12 - vanilla RT kernel

* Tue Dec 24 2024 Yann Collette <ycollette.nospam@free.fr> - 5.10.231-rt123-12
- update to 5.10.231-rt123-12 - vanilla RT kernel

* Tue Nov 26 2024 Yann Collette <ycollette.nospam@free.fr> - 5.10.229-rt121-12
- update to 5.10.229-rt121-12 - vanilla RT kernel

* Mon Sep 16 2024 Yann Collette <ycollette.nospam@free.fr> - 5.10.225-rt117-12
- update to 5.10.225-rt117-12 - vanilla RT kernel

* Fri Sep 06 2024 Yann Collette <ycollette.nospam@free.fr> - 5.10.224-rt116-12
- update to 5.10.224-rt116-12 - vanilla RT kernel

* Thu Aug 08 2024 Yann Collette <ycollette.nospam@free.fr> - 5.10.223-rt115-12
- update to 5.10.223-rt115-12 - vanilla RT kernel
...
* Sat Jun 29 2024 Yann Collette <ycollette.nospam@free.fr> - 5.10.219-rt111-12
- update to 5.10.219-rt111-12 - vanilla RT kernel
