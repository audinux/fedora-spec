...
dd if=/dev/zero of=$RPM_BUILD_ROOT/boot/initramfs-%{kver}-rt-stable%{krt}%{fcver}.img bs=1M count=20

%ifarch ia64
  cp $KBUILD_IMAGE $RPM_BUILD_ROOT/boot/efi/vmlinuz-%{kver}-rt-stable%{krt}%{fcver}
  chmod a+x $RPM_BUILD_ROOT/boot/efi/vmlinuz-%{kver}-rt-stable%{krt}%{fcver}
  ln -s efi/vmlinuz-%{kver}-stable-%{krt}%{fcver} $RPM_BUILD_ROOT/boot/
%else
  cp $KBUILD_IMAGE $RPM_BUILD_ROOT/boot/vmlinuz-%{kver}-rt-stable%{krt}%{fcver}
  chmod a+x $RPM_BUILD_ROOT/boot/vmlinuz-%{kver}-rt-stable%{krt}%{fcver}
%endif

make %{?_smp_mflags} INSTALL_HDR_PATH=$RPM_BUILD_ROOT/usr KBUILD_SRC= headers_install
cp System.map $RPM_BUILD_ROOT/boot/System.map-%{kver}-rt-stable%{krt}%{fcver}
cp .config    $RPM_BUILD_ROOT/boot/config-%{kver}-rt-stable%{krt}%{fcver}

cp $RPM_BUILD_ROOT/boot/vmlinuz-%{kver}-rt-stable%{krt}%{fcver} $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/vmlinuz

rm -f $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build
rm -f $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/source

mkdir -p $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build
(cd $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver} ; ln -s build source)

# dirs for additional modules per module-init-tools, kbuild/modules.txt
mkdir -p $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/extra
mkdir -p $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/internal
mkdir -p $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/updates
...
cp --parents `find  -type f -name "Makefile*" -o -name "Kconfig*"` $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build
cp Module.symvers $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build
cp System.map $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build
if [ -s Module.markers ]; then
  cp Module.markers $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build
fi
...
mkdir -p $RPM_BUILD_ROOT/usr/src/kernels/%{kver}-rt-stable%{krt}%{fcver}
mv $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build $RPM_BUILD_ROOT/$DevelDir
...
ln -sf $DevelDir $RPM_BUILD_ROOT/lib/modules/%{kver}-rt-stable%{krt}%{fcver}/build
...
tar $EXCLUDES -cf- . | (cd $RPM_BUILD_ROOT/usr/src/kernels/%{kver}-rt-stable%{krt}%{fcver}; tar xvf -)
