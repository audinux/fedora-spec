...
jq '.dpf.plugin_formats |= map(select(. != "au"))' WSTD_EQ.json > tmp.$$.json && mv tmp.$$.json WSTD_EQ.json
...
cp -ra bin/WSTD_EQ.lv2 %{buildroot}/%{_libdir}/lv2/
cp bin/WSTD_EQ-vst.so %{buildroot}/%{_libdir}/vst/
cp -ra bin/WSTD_EQ.vst3 %{buildroot}/%{_libdir}/vst3/
cp bin/WSTD_EQ.clap %{buildroot}/%{_libdir}/clap/
