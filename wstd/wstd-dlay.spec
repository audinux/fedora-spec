...
# ./wstd-source.sh wstd-dlay v1.1.1
...
jq '.dpf.plugin_formats |= map(select(. != "au"))' WSTD_DLAY.json > tmp.$$.json && mv tmp.$$.json WSTD_DLAY.json
...
cp -ra bin/WSTD_DLAY.lv2 %{buildroot}/%{_libdir}/lv2/
cp bin/WSTD_DLAY-vst.so %{buildroot}/%{_libdir}/vst/
cp -ra bin/WSTD_DLAY.vst3 %{buildroot}/%{_libdir}/vst3/
cp bin/WSTD_DLAY.clap %{buildroot}/%{_libdir}/clap/
