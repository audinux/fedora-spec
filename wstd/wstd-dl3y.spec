...
jq '.dpf.plugin_formats |= map(select(. != "au"))' WSTD_DL3Y.json > tmp.$$.json && mv tmp.$$.json WSTD_DL3Y.json
...
cp -ra bin/WSTD_DL3Y.lv2 %{buildroot}/%{_libdir}/lv2/
cp bin/WSTD_DL3Y-vst.so %{buildroot}/%{_libdir}/vst/
cp -ra bin/WSTD_DL3Y.vst3 %{buildroot}/%{_libdir}/vst3/
cp bin/WSTD_DL3Y.clap %{buildroot}/%{_libdir}/clap/
