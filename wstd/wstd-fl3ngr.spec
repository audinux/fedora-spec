...
Version: 1.1.1
Release: 1%{?dist}
Summary: A multiband flanger plugin
License: GPL-3.0-or-later
URL: https://github.com/Wasted-Audio/wstd-fl3ngr
...
# ./wstd-source.sh wstd-fl3ngr v1.1.1

Source0: wstd-fl3ngr.tar.gz
...
jq '.dpf.plugin_formats |= map(select(. != "au"))' WSTD_FL3NGR.json > tmp.$$.json && mv tmp.$$.json WSTD_FL3NGR.json
...
%make_build PLUGIN=wstd_fl3ngr PREFIX=/usr LIBDIR=%{_libdir} SKIP_STRIPPING=true
...
cp -ra bin/WSTD_FL3NGR.lv2 %{buildroot}/%{_libdir}/lv2/
cp bin/WSTD_FL3NGR-vst.so %{buildroot}/%{_libdir}/vst/
cp -ra bin/WSTD_FL3NGR.vst3 %{buildroot}/%{_libdir}/vst3/
cp bin/WSTD_FL3NGR.clap %{buildroot}/%{_libdir}/clap/
