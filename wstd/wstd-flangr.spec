...
# ./wstd-source.sh wstd-flangr v1.1.1
...
jq '.dpf.plugin_formats |= map(select(. != "au"))' WSTD_FLANGR.json > tmp.$$.json && mv tmp.$$.json WSTD_FLANGR.json
...
cp -ra bin/WSTD_FLANGR.lv2 %{buildroot}/%{_libdir}/lv2/
cp bin/WSTD_FLANGR-vst.so %{buildroot}/%{_libdir}/vst/
cp -ra bin/WSTD_FLANGR.vst3 %{buildroot}/%{_libdir}/vst3/
cp bin/WSTD_FLANGR.clap %{buildroot}/%{_libdir}/clap/
