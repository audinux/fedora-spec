...
# ./wstd-source.sh wstd-smthr v1.1.1

Source0: wstd-smthr.tar.gz
...
%description
Simple soft-clipping overdrive plugin
...
jq '.dpf.plugin_formats |= map(select(. != "au"))' WSTD_SMTHR.json > tmp.$$.json && mv tmp.$$.json WSTD_SMTHR.json
...
%make_build PLUGIN=wstd_smthr PREFIX=/usr LIBDIR=%{_libdir} SKIP_STRIPPING=true
...
cp -ra bin/WSTD_SMTHR.lv2 %{buildroot}/%{_libdir}/lv2/
cp bin/WSTD_SMTHR-vst.so %{buildroot}/%{_libdir}/vst/
cp -ra bin/WSTD_SMTHR.vst3 %{buildroot}/%{_libdir}/vst3/
cp bin/WSTD_SMTHR.clap %{buildroot}/%{_libdir}/clap/
