...
%global commit0 8f5dcbc7d6a040f05856d331743d683daae50357
...
Name: rack-v2-RoomiFields
Version: 2.1.2
Release: 2%{?dist}
Summary: RoomiFields plugin for Rack
License: GPL-2.0-or-later
URL: https://github.com/roomi-fields/vcv-osc
...
Source0: Rack.tar.gz
Source1: https://github.com/roomi-fields/vcv-osc/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: RoomiFields_plugin.json
...
%description
RoomiFields plugin for Rack.
Remote-controls the whole Rack patch over OSC: set/get any parameter (by id or name), add/remove cables and modules, load/save presets, dump patch state, and serve an OSCQuery namespace for TouchOSC/Open Stage Control. Passthrough-compatible with osc-bridge.
...
mkdir RoomiFields_plugin
tar xvfz %{SOURCE1} --directory=RoomiFields_plugin --strip-components=1

cp -n %{SOURCE2} RoomiFields_plugin/plugin.json || true

%build

cd RoomiFields_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/RoomiFields/
cp -r RoomiFields_plugin/dist/RoomiFields/* %{buildroot}%{_libexecdir}/Rack2/plugins/RoomiFields/
