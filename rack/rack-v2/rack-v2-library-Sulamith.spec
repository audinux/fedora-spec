...
%global commit0 2565a74e48a2e97715d1728eb21936f9482a27d6
...
Source0: Rack.tar.gz
Source1: https://github.com/JohannAsbjoernson/Sulamith/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: Sulamith_plugin.json
...
Trigger and Gate utility with two OR-logic inputs. Features probability and custom gate length (0.2-10s). A/B section works as Toggle, Constant Voltage (1-10v), Bi- and Unipolar CV generation with range options. Context menu: Re-Trig can be disabled (i.e. to sync CV generation with custom length gates), Slew can be enabled for custom gates & cv gen.
...
mkdir Sulamith_plugin
tar xvfz %{SOURCE1} --directory=Sulamith_plugin --strip-components=1

cp -n %{SOURCE2} Sulamith_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/Sulamith/
cp -r Sulamith_plugin/dist/Sulamith/* %{buildroot}%{_libexecdir}/Rack2/plugins/Sulamith/
