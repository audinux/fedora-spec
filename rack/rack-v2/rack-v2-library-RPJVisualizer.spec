...
URL:     https://github.com/kockie69/Visualizer
...
Source0: Rack.tar.gz
Source1: https://github.com/kockie69/Visualizer/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: RPJVisualizer_plugin.json
...
%description
RPJVisualizer plugin for Rack.
Psycho visualizer, shown in a separate display. 
...
mkdir RPJVisualizer_plugin
tar xvfz %{SOURCE1} --directory=RPJVisualizer_plugin --strip-components=1

cp -n %{SOURCE2} RPJVisualizer_plugin/plugin.json || true

%build

cd RPJVisualizer_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/RPJVisualizer/
cp -r RPJVisualizer_plugin/dist/RPJVisualizer/* %{buildroot}%{_libexecdir}/Rack2/plugins/RPJVisualizer/
