...
%description
C1-ChannelStrip plugin for Rack.
Input with VCA, high/low cuts, and phase invert
...
mkdir C1-ChannelStrip_plugin
tar xvfz %{SOURCE1} --directory=C1-ChannelStrip_plugin --strip-components=1

cp -n %{SOURCE2} C1-ChannelStrip_plugin/plugin.json || true

%build

cd C1-ChannelStrip_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/C1-ChannelStrip/
cp -r C1-ChannelStrip_plugin/dist/C1-ChannelStrip/* %{buildroot}%{_libexecdir}/Rack2/plugins/C1-ChannelStrip/
