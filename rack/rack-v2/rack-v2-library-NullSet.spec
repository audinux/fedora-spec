...
%global commit0 3637d33896d66c4aa0abb0e0e7e3e42f1fd3acb9
...
Source0: Rack.tar.gz
Source1: https://github.com/nicholas-j-denning/NullSet/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: NullSet_plugin.json
...
%description
NullSet plugin for Rack.
Generate melodies using an aliasing algorithm
...
mkdir NullSet_plugin
tar xvfz %{SOURCE1} --directory=NullSet_plugin --strip-components=1

cp -n %{SOURCE2} NullSet_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/NullSet/
cp -r NullSet_plugin/dist/NullSet/* %{buildroot}%{_libexecdir}/Rack2/plugins/NullSet/
