...
%global commit0 4120a7436b87afe35f9e776ded2880f8bbd27d4f
...
Name: rack-v2-MissingMileModular
Version: 2.0.0
Release: 2%{?dist}
Summary: MissingMileModular plugin for Rack
License: GPL-2.0-or-later
URL: https://github.com/kuangmk11/Doubleplus_Pulses_VCV
...
Source0: Rack.tar.gz
Source1: https://github.com/kuangmk11/Doubleplus_Pulses_VCV/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: MissingMileModular_plugin.json
...
%description
MissingMileModular plugin for Rack.
Performance gate router for the Music Thing Turing Machine: eight channels, each routed to one of two OR/AND merge buses.
...
mkdir MissingMileModular_plugin
tar xvfz %{SOURCE1} --directory=MissingMileModular_plugin --strip-components=1

cp -n %{SOURCE2} MissingMileModular_plugin/plugin.json || true

%build

cd MissingMileModular_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/MissingMileModular/
cp -r MissingMileModular_plugin/dist/MissingMileModular/* %{buildroot}%{_libexecdir}/Rack2/plugins/MissingMileModular/
