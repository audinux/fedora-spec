...
Source0: Rack.tar.gz
Source1: https://github.com/shortwavlabs/vcv-guitar-collection/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: swv-guitar-tools_plugin.json
...
%description
swv-guitar-tools plugin for Rack.
...
mkdir swv-guitar-tools_plugin
tar xvfz %{SOURCE1} --directory=swv-guitar-tools_plugin --strip-components=1

cp -n %{SOURCE2} swv-guitar-tools_plugin/plugin.json || true

%build

cd swv-guitar-tools_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/swv-guitar-tools/
cp -r swv-guitar-tools_plugin/dist/swv-guitar-tools/* %{buildroot}%{_libexecdir}/Rack2/plugins/swv-guitar-tools/
