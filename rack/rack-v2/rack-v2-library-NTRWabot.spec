...
URL: https://github.com/nevertrustrobots/NTRWabot
...
Source0: Rack.tar.gz
Source1: https://github.com/nevertrustrobots/NTRWabot/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: NTRWabot_plugin.json
...
%description
NTRWabot plugin for Rack.
HTTP bridge that lets Claude AI read and control your VCV Rack patch in real time. Connect it to Claude using the MCP Server + Skill available at https://www.patreon.com/c/nevertrustrobots
...
mkdir NTRWabot_plugin
tar xvfz %{SOURCE1} --directory=NTRWabot_plugin --strip-components=1

cp -n %{SOURCE2} NTRWabot_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/NTRWabot/
cp -r NTRWabot_plugin/dist/NTRWabot/* %{buildroot}%{_libexecdir}/Rack2/plugins/NTRWabot/
