...
Source0: Rack.tar.gz
Source1: https://github.com/brer-rabbit/zoxnoxious-virtual//archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: Zoxnoxious_plugin.json
...
mkdir Zoxnoxious_plugin
tar xvfz %{SOURCE1} --directory=Zoxnoxious_plugin --strip-components=1

cp -n %{SOURCE2} Zoxnoxious_plugin/plugin.json || true

%build

cd Zoxnoxious_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/Zoxnoxious/
cp -r Zoxnoxious_plugin/dist/Zoxnoxious/* %{buildroot}%{_libexecdir}/Rack2/plugins/Zoxnoxious/
