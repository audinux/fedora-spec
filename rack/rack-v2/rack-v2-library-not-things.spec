...
%global commit0 25c0ffa2aabda800868c8f11cd6712e9c729cdc9
...
Source0: Rack.tar.gz
Source1: https://github.com/not-things-modular/not-things-vcv/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: not-things_plugin.json
...
%description
not-things plugin for Rack.
Limiter and sorter of (note) voltages
...
mkdir not-things_plugin
tar xvfz %{SOURCE1} --directory=not-things_plugin --strip-components=1

cp -n %{SOURCE2} not-things_plugin/plugin.json || true

%build

cd not-things_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/not-things/
cp -r not-things_plugin/dist/not-things/* %{buildroot}%{_libexecdir}/Rack2/plugins/not-things/
