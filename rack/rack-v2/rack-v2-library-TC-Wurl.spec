...
%global commit0 5d573b7c3bdb2f20e8d98dab94ccd111d204513b
...
Source0: Rack.tar.gz
Source1: https://github.com/Eurikon/TC-Wurl/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: TC-Wurl_plugin.json
...
%description
TC-Wurl plugin for Rack.
Physically-modeled electric piano
...
mkdir TC-Wurl_plugin
tar xvfz %{SOURCE1} --directory=TC-Wurl_plugin --strip-components=1

cp -n %{SOURCE2} TC-Wurl_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/TC-Wurl/
cp -r TC-Wurl_plugin/dist/TC-Wurl/* %{buildroot}%{_libexecdir}/Rack2/plugins/TC-Wurl/
