...
Source0: Rack.tar.gz
Source1: https://github.com/grough/lilac-modules-vcv/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: LilacModules_plugin.json
...
mkdir LilacModules_plugin
tar xvfz %{SOURCE1} --directory=LilacModules_plugin --strip-components=1

cp -n %{SOURCE2} LilacModules_plugin/plugin.json || true

%build

cd LilacModules_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/LilacModules/
cp -r LilacModules_plugin/dist/LilacModules/* %{buildroot}%{_libexecdir}/Rack2/plugins/LilacModules/
