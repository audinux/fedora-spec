...
%global commit0 f0d0e8e830f9f3bb27020d997205b67b4a8570fc
%global gittag0 2.1.6.0
...
Source0: Rack.tar.gz
Source1: https://github.com/michelezaccagnini/NLS/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: NLS_plugin.json
...
%description
NLS plugin for Rack.
detect intersections between LFOs to generate triggers
...
mkdir NLS_plugin
tar xvfz %{SOURCE1} --directory=NLS_plugin --strip-components=1

cp -n %{SOURCE2} NLS_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/NLS/
cp -r NLS_plugin/dist/NLS/* %{buildroot}%{_libexecdir}/Rack2/plugins/NLS/

%files
%{_libexecdir}/*

%changelog
* Tue Nov 30 2021 Yann Collette <ycollette.nospam@free.fr> - 2.1.6.0-1
- initial specfile
