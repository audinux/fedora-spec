...
%global commit0 56bf82b2fcb9fe833d3ea7cc43b3b4f44f8f63ef
...
Name:    rack-v2-HetrickCVGPL
Version: 2.0.0
Release: 2%{?dist}
Summary: HetrickCVGPL plugin for Rack
License: GPL-2.0-or-later
URL:     https://github.com/mhetrick/phasorbeatgen
...
Source0: Rack.tar.gz
Source1: https://github.com/mhetrick/phasorbeatgen/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: HetrickCVGPL_plugin.json
...
%description
HetrickCVGPL plugin for Rack.
Phasor-driven topographic drum sequencer based on Mutable Instruments Grids. Uses phasor input (0-10V) for playback, enabling reverse, scrub, and non-linear pattern navigation.
...
mkdir HetrickCVGPL_plugin
tar xvfz %{SOURCE1} --directory=HetrickCVGPL_plugin --strip-components=1

cp -n %{SOURCE2} HetrickCVGPL_plugin/plugin.json || true

%build

cd HetrickCVGPL_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/HetrickCVGPL/
cp -r HetrickCVGPL_plugin/dist/HetrickCVGPL/* %{buildroot}%{_libexecdir}/Rack2/plugins/HetrickCVGPL/
