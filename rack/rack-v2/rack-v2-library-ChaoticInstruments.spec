...
%global commit0 fa31e889a00db1bbf3a19edd2af8c8722a8613e8
...
Name: rack-v2-ChaoticInstruments
Version: 2.0.0
Release: 2%{?dist}
Summary: ChaoticInstruments plugin for Rack
License: GPL-2.0-or-later
URL: https://github.com/songdsp/chaotic-vcv
...
Source0: Rack.tar.gz
Source1: https://github.com/songdsp/chaotic-vcv/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: ChaoticInstruments_plugin.json
...
%description
ChaoticInstruments plugin for Rack.
FFT spectral freeze: captures the incoming spectrum and sustains it as a frozen pad.
...
mkdir ChaoticInstruments_plugin
tar xvfz %{SOURCE1} --directory=ChaoticInstruments_plugin --strip-components=1

cp -n %{SOURCE2} ChaoticInstruments_plugin/plugin.json || true

%build

cd ChaoticInstruments_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/ChaoticInstruments/
cp -r ChaoticInstruments_plugin/dist/ChaoticInstruments/* %{buildroot}%{_libexecdir}/Rack2/plugins/ChaoticInstruments/
