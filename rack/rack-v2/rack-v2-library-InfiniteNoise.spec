...
URL: https://github.com/pellelil/InfNoiseRack
...
Source0: Rack.tar.gz
Source1: https://github.com/pellelil/InfNoiseRack/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: InfiniteNoise_plugin.json
...
mkdir InfiniteNoise_plugin
tar xvfz %{SOURCE1} --directory=InfiniteNoise_plugin --strip-components=1

cp -n %{SOURCE2} InfiniteNoise_plugin/plugin.json || true

%build

cd InfiniteNoise_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/InfiniteNoise/
cp -r InfiniteNoise_plugin/dist/InfiniteNoise/* %{buildroot}%{_libexecdir}/Rack2/plugins/InfiniteNoise/
