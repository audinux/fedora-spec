...
Source0: Rack.tar.gz
Source1: https://github.com/fairplay/vcv-rack-plugins/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: TyrannosaurusRu_plugin.json
...
mkdir TyrannosaurusRu_plugin
tar xvfz %{SOURCE1} --directory=TyrannosaurusRu_plugin --strip-components=1

cp -n %{SOURCE2} TyrannosaurusRu_plugin/plugin.json || true

%build

cd TyrannosaurusRu_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/TyrannosaurusRu/
cp -r TyrannosaurusRu_plugin/dist/TyrannosaurusRu/* %{buildroot}%{_libexecdir}/Rack2/plugins/TyrannosaurusRu/
