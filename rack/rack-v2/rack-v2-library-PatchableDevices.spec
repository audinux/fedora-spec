...
URL:     https://github.com/nullJaX/vcvrack-patchable-devices
...
Source0: Rack.tar.gz
Source1: https://github.com/nullJaX/vcvrack-patchable-devices/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: PatchableDevices_plugin.json
...
%description
PatchableDevices plugin for Rack.
...
mkdir PatchableDevices_plugin
tar xvfz %{SOURCE1} --directory=PatchableDevices_plugin --strip-components=1

cp -n %{SOURCE2} PatchableDevices_plugin/plugin.json || true

%build

cd PatchableDevices_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/PatchableDevices/
cp -r PatchableDevices_plugin/dist/PatchableDevices/* %{buildroot}%{_libexecdir}/Rack2/plugins/PatchableDevices/
