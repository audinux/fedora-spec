...
URL:     https://github.com/scanner-darkly/eightfold
...
Source0: Rack.tar.gz
Source1: https://github.com/scanner-darkly/eightfold/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: eightfold_plugin.json
...
mkdir eightfold_plugin
tar xvfz %{SOURCE1} --directory=eightfold_plugin --strip-components=1

cp -n %{SOURCE2} eightfold_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/eightfold/
cp -r eightfold_plugin/dist/eightfold/* %{buildroot}%{_libexecdir}/Rack2/plugins/eightfold/
