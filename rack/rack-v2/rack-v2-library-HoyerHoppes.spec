...
%global commit0 752e866a532c45ebb1f143487cf6796732b42c2a
...
Name:    rack-v2-HoyerHoppes
Version: 2.0.0
Release: 2%{?dist}
Summary: HoyerHoppes plugin for Rack
License: GPL-2.0-or-later
URL:     https://github.com/jakulley/HoyerHoppes
...
Source0: Rack.tar.gz
Source1: https://github.com/jakulley/HoyerHoppes/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: HoyerHoppes_plugin.json
...
%description
HoyerHoppes plugin for Rack.
Create a list of clock ratios, then scan through the list.
...
mkdir HoyerHoppes_plugin
tar xvfz %{SOURCE1} --directory=HoyerHoppes_plugin --strip-components=1

cp -n %{SOURCE2} HoyerHoppes_plugin/plugin.json || true

%build

cd HoyerHoppes_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/HoyerHoppes/
cp -r HoyerHoppes_plugin/dist/HoyerHoppes/* %{buildroot}%{_libexecdir}/Rack2/plugins/HoyerHoppes/
