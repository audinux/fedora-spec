...
%global commit0 25cd56b5e81b9b892a5f17658d659e790ec189f7
...
Name:    rack-v2-WordGenerator
Version: 2.0.0
Release: 2%{?dist}
Summary: WordGenerator plugin for Rack
...
Source0: Rack.tar.gz
Source1: https://github.com/sindarknave/word-generator/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: WordGenerator_plugin.json
...
%description
WordGenerator plugin for Rack.
Digital sequencer inspired by the HP 8006A Word Generator
...
mkdir WordGenerator_plugin
tar xvfz %{SOURCE1} --directory=WordGenerator_plugin --strip-components=1

cp -n %{SOURCE2} WordGenerator_plugin/plugin.json || true

%build

cd WordGenerator_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/WordGenerator/
cp -r WordGenerator_plugin/dist/WordGenerator/* %{buildroot}%{_libexecdir}/Rack2/plugins/WordGenerator/
