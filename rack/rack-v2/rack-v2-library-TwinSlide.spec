...
URL:     https://github.com/Eurikon/Twin-Slide
...
Source0: Rack.tar.gz
Source1: https://github.com/Eurikon/Twin-Slide/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: TwinSlide_plugin.json
...
%description
TwinSlide plugin for Rack.
Dual acid bass synthesizer
...
mkdir TwinSlide_plugin
tar xvfz %{SOURCE1} --directory=TwinSlide_plugin --strip-components=1

cp -n %{SOURCE2} TwinSlide_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/TwinSlide/
cp -r TwinSlide_plugin/dist/TwinSlide/* %{buildroot}%{_libexecdir}/Rack2/plugins/TwinSlide/
