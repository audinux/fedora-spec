...
%global commit0 4b939879adfa022eec4cc95bdaba4b09783b86f1
...
Source0: Rack.tar.gz
Source1: https://github.com/BooleanMuse/Photuri-Electronics/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: PhoturiElectronics_plugin.json
...
%description
PhoturiElectronics plugin for Rack.
...
mkdir PhoturiElectronics_plugin
tar xvfz %{SOURCE1} --directory=PhoturiElectronics_plugin --strip-components=1

cp -n %{SOURCE2} PhoturiElectronics_plugin/plugin.json || true

%build

cd PhoturiElectronics_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/PhoturiElectronics/
cp -r PhoturiElectronics_plugin/dist/PhoturiElectronics/* %{buildroot}%{_libexecdir}/Rack2/plugins/PhoturiElectronics/
