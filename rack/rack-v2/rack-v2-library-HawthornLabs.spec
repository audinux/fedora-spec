...
URL:     https://github.com/AFHawthorn/HawthornLabs
...
Source0: Rack.tar.gz
Source1: https://github.com/AFHawthorn/HawthornLabs/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: HawthornLabs_plugin.json
...
%description
HawthornLabs plugin for Rack.
LFOs from the movement of the solar system
...
mkdir HawthornLabs_plugin
tar xvfz %{SOURCE1} --directory=HawthornLabs_plugin --strip-components=1

cp -n %{SOURCE2} HawthornLabs_plugin/plugin.json || true

%build

cd HawthornLabs_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/HawthornLabs/
cp -r HawthornLabs_plugin/dist/HawthornLabs/* %{buildroot}%{_libexecdir}/Rack2/plugins/HawthornLabs/
