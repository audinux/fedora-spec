...
%global commit0 748e49ca55c054df66fb24158ed8023b27a80550
...
Source0: Rack.tar.gz
Source1: https://github.com/moffenzeefmodular/VCVrack-submission/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: Moffenzeef_plugin.json
...
mkdir Moffenzeef_plugin
tar xvfz %{SOURCE1} --directory=Moffenzeef_plugin --strip-components=1

cp -n %{SOURCE2} Moffenzeef_plugin/plugin.json || true

%build

cd Moffenzeef_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/Moffenzeef/
cp -r Moffenzeef_plugin/dist/Moffenzeef/* %{buildroot}%{_libexecdir}/Rack2/plugins/Moffenzeef/
