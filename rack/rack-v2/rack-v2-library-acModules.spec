...
%global commit0 db1e3520d51a570f5dc95ea1ba70f5ba05307d97
...
Source0: Rack.tar.gz
Source1: https://github.com/gwygonik/acModules/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: acModules_plugin.json
...
mkdir acModules_plugin
tar xvfz %{SOURCE1} --directory=acModules_plugin --strip-components=1

cp -n %{SOURCE2} acModules_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/acModules/
cp -r acModules_plugin/dist/acModules/* %{buildroot}%{_libexecdir}/Rack2/plugins/acModules/
