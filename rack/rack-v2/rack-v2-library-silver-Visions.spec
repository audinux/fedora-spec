...
URL:     https://github.com/silver-ag/visions
...
Source0: Rack.tar.gz
Source1: https://github.com/silver-ag/visions/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: silver-Visions_plugin.json
...
%description
silver-Visions plugin for Rack.
video synth output
...
mkdir silver-Visions_plugin
tar xvfz %{SOURCE1} --directory=silver-Visions_plugin --strip-components=1

cp -n %{SOURCE2} silver-Visions_plugin/plugin.json || true

%build

cd silver-Visions_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/silver-Visions/
cp -r silver-Visions_plugin/dist/silver-Visions/* %{buildroot}%{_libexecdir}/Rack2/plugins/silver-Visions/
