...
%global commit0 b9d77ecc8f013958813fc22ff7eba9feb31cb69e
...
Name: rack-v2-BerserkAudio
Version: 2.1.0
Release: 2%{?dist}
Summary: BerserkAudio plugin for Rack
License: GPL-2.0-or-later
URL: https://github.com/NicolasMurphy/BerserkAudio
...
Source0: Rack.tar.gz
Source1: https://github.com/NicolasMurphy/BerserkAudio/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: BerserkAudio_plugin.json
...
%description
BerserkAudio plugin for Rack.
Comb reverb with self-oscillating feedback.
...
mkdir BerserkAudio_plugin
tar xvfz %{SOURCE1} --directory=BerserkAudio_plugin --strip-components=1

cp -n %{SOURCE2} BerserkAudio_plugin/plugin.json || true

%build

cd BerserkAudio_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/BerserkAudio/
cp -r BerserkAudio_plugin/dist/BerserkAudio/* %{buildroot}%{_libexecdir}/Rack2/plugins/BerserkAudio/
