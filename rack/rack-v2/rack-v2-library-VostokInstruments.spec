...
Source0: Rack.tar.gz
Source1: https://github.com/Vostok-Instruments/Vostok_VCVRack/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: VostokInstruments_plugin.json
...
%description
VostokInstruments plugin for Rack.
Analog Morphing Router
...
mkdir VostokInstruments_plugin
tar xvfz %{SOURCE1} --directory=VostokInstruments_plugin --strip-components=1

cp -n %{SOURCE2} VostokInstruments_plugin/plugin.json || true

%build

cd VostokInstruments_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/VostokInstruments/
cp -r VostokInstruments_plugin/dist/VostokInstruments/* %{buildroot}%{_libexecdir}/Rack2/plugins/VostokInstruments/
