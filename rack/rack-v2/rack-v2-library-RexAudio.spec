...
%global commit0 5a41c22847cd4f794c2b4354308377f979a369cb
...
Name:    rack-v2-RexAudio
Version: 2.0.0
Release: 2%{?dist}
Summary: RexAudio plugin for Rack
License: GPL-2.0-or-later
URL:     https://github.com/RexAudio/VCV
...
Source0: Rack.tar.gz
Source1: https://github.com/RexAudio/VCV/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: RexAudio_plugin.json
...
%description
RexAudio plugin for Rack.
DX-style FM/PM effect with audio input
...
mkdir RexAudio_plugin
tar xvfz %{SOURCE1} --directory=RexAudio_plugin --strip-components=1

cp -n %{SOURCE2} RexAudio_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/RexAudio/
cp -r RexAudio_plugin/dist/RexAudio/* %{buildroot}%{_libexecdir}/Rack2/plugins/RexAudio/
