...
%global commit0 de21c29c6930bb074edc385ccf1e1679b93773d0
%global gittag0 2.19.0
...
URL: https://github.com/submitaudio/submit-vcv-modules
...
Source0: Rack.tar.gz
Source1: https://github.com/submitaudio/submit-vcv-modules/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: Submit_plugin.json
...
%description
Submit plugin for Rack.
West Coast voice with oscillator, wavefolder, slope and contour envelope.
...
mkdir Submit_plugin
tar xvfz %{SOURCE1} --directory=Submit_plugin --strip-components=1

cp -n %{SOURCE2} Submit_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/Submit/
cp -r Submit_plugin/dist/Submit/* %{buildroot}%{_libexecdir}/Rack2/plugins/Submit/

%files
%{_libexecdir}/*

%changelog
* Tue Nov 30 2021 Yann Collette <ycollette.nospam@free.fr> - 2.19.0-1
- initial specfile
