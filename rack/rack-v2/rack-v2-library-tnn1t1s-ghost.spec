...
Source0: Rack.tar.gz
Source1: https://github.com/tnn1t1s/tnn1t1s-ghost/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: tnn1t1s-ghost_plugin.json
...
%description
tnn1t1s-ghost plugin for Rack.
909-inspired kick voice. Struck bridged-T resonator model with damped sine body, pitch-envelope glide, bandpassed click transient, and tanh drive. Works standalone; comes alive with GHOST CTRL.
...
mkdir tnn1t1s-ghost_plugin
tar xvfz %{SOURCE1} --directory=tnn1t1s-ghost_plugin --strip-components=1

cp -n %{SOURCE2} tnn1t1s-ghost_plugin/plugin.json || true

%build

cd tnn1t1s-ghost_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/tnn1t1s-ghost/
cp -r tnn1t1s-ghost_plugin/dist/tnn1t1s-ghost/* %{buildroot}%{_libexecdir}/Rack2/plugins/tnn1t1s-ghost/
