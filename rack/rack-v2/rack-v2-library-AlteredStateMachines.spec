...
Source0: Rack.tar.gz
Source1: https://codeberg.org/alteredstatemachines/alteredstatemachines-vcv/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: AlteredStateMachines_plugin.json
...
%description
AlteredStateMachines plugin for Rack.
Nonlinear 2 Channel Counterpoint Voltage Sequencer (Aristotle hardware clone)
...
mkdir AlteredStateMachines_plugin
tar xvfz %{SOURCE1} --directory=AlteredStateMachines_plugin --strip-components=1

cp -n %{SOURCE2} AlteredStateMachines_plugin/plugin.json || true

%build

cd AlteredStateMachines_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/AlteredStateMachines/
cp -r AlteredStateMachines_plugin/dist/AlteredStateMachines/* %{buildroot}%{_libexecdir}/Rack2/plugins/AlteredStateMachines/
