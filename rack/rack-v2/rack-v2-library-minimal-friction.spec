...
URL:     https://github.com/ab9st8/minimal-friction
...
Source0: Rack.tar.gz
Source1: https://github.com/ab9st8/minimal-friction/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: minimal-friction_plugin.json
...
%description
minimal-friction plugin for Rack.
...
mkdir minimal-friction_plugin
tar xvfz %{SOURCE1} --directory=minimal-friction_plugin --strip-components=1

cp -n %{SOURCE2} minimal-friction_plugin/plugin.json || true

%build

cd minimal-friction_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/minimal-friction/
cp -r minimal-friction_plugin/dist/minimal-friction/* %{buildroot}%{_libexecdir}/Rack2/plugins/minimal-friction/
