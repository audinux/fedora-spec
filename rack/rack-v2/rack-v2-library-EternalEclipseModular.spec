...
%description
EternalEclipseModular plugin for Rack.
Four-element wavetable synth voice — a pentagram of AIR, WATER, EARTH, and FIRE oscillators with elemental-ring cross-modulation, opposing-pair ring mod, SPIRIT chord mode, 72 daemon algorithms, ladder lowpass filter, and stereo delay/diffusion.
...
mkdir EternalEclipseModular_plugin
tar xvfz %{SOURCE1} --directory=EternalEclipseModular_plugin --strip-components=1

cp -n %{SOURCE2} EternalEclipseModular_plugin/plugin.json || true

%build

cd EternalEclipseModular_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/EternalEclipseModular/
cp -r EternalEclipseModular_plugin/dist/EternalEclipseModular/* %{buildroot}%{_libexecdir}/Rack2/plugins/EternalEclipseModular/
