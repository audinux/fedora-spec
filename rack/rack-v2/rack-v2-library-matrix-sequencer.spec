...
%global commit0 b6424e57608690e9b165af9e7532f2b6c37e566b
...
Source0: Rack.tar.gz
Source1: https://github.com/Retr0-code/Matrix-Sequncer/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: matrix-sequencer_plugin.json
...
%description
matrix-sequencer plugin for Rack.
...
mkdir matrix-sequencer_plugin
tar xvfz %{SOURCE1} --directory=matrix-sequencer_plugin --strip-components=1

cp -n %{SOURCE2} matrix-sequencer_plugin/plugin.json || true

%build

cd matrix-sequencer_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/matrix-sequencer/
cp -r matrix-sequencer_plugin/dist/matrix-sequencer/* %{buildroot}%{_libexecdir}/Rack2/plugins/matrix-sequencer/
