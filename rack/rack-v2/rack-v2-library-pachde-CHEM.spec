...
%global commit0 fbdb29e6cfc1dc31ca78a116d6f4254f847a7349
...
Source0: Rack.tar.gz
Source1: https://github.com/Paul-Dempsey/pachde-CHEM/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: pachde-CHEM_plugin.json
...
%description
pachde-CHEM plugin for Rack.
CHEM Core for Haken EaganMatrix
...
mkdir pachde-CHEM_plugin
tar xvfz %{SOURCE1} --directory=pachde-CHEM_plugin --strip-components=1

cp -n %{SOURCE2} pachde-CHEM_plugin/plugin.json || true

%build

cd pachde-CHEM_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install

mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/pachde-CHEM/
cp -r pachde-CHEM_plugin/dist/pachde-CHEM/* %{buildroot}%{_libexecdir}/Rack2/plugins/pachde-CHEM/
