...
URL:     https://github.com/shortwavlabs/vcv-tapestry
...
Source0: Rack.tar.gz
Source1: https://github.com/shortwavlabs/vcv-tapestry/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: tapestry_plugin.json
...
%description
tapestry plugin for Rack.
Granular microsound processor inspired by musique concrète - manipulate audio reels with splices, grains, and morph controls
...
mkdir tapestry_plugin
tar xvfz %{SOURCE1} --directory=tapestry_plugin --strip-components=1

cp -n %{SOURCE2} tapestry_plugin/plugin.json || true
...
mkdir -p %{buildroot}%{_libexecdir}/Rack2/plugins/tapestry/
cp -r tapestry_plugin/dist/tapestry/* %{buildroot}%{_libexecdir}/Rack2/plugins/tapestry/
